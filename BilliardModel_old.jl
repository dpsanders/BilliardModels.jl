...
function collision_time(p::Particle, disc::Disc, outside::Bool=true)
  ...
  if outside
    return (-B - √(discriminant)) / A  # suppose outside disc
  end
...
type BilliardTable
  ...
  periodizing_rules::Dict{Int, Int}
end
...
function make_Lorentz_table(radius)
    ...
    periodizing_rules = {2=>4, 4=>2, 3=>5, 5=>3}  # which periodic boundaries map to which

    return BilliardTable(boundaries, periodizing_rules)
